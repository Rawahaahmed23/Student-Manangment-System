...
import { TrendingUp, TrendingDown, ChevronLeft, ChevronRight, Trash2, Eye, AlertTriangle, X } from "lucide-react";
...
function getPaginationRange(current, total) {
  ...
  else if (current >= total - 3) return [1, "...", total - 4, total - 3, total - 2, total - 1, total];
  else return [1, "...", current - 1, current, current + 1, "...", total];
}
...
const handleDeleteConfirm = (studentId, year) => {
  if (!studentId || !year) return;
  onDeleteFees?.(studentId, year);
  setDeleteTarget({ student: null, year: currentYear });
};
    ...
    <div className="flex items-center gap-1.5">
      ...
      {student.feeStatus !== "Paid" ? (
        <button
          ...
          className="px-2.5 py-1.5 bg-green-100 text-green-700 rounded-lg flex items-center gap-1 text-xs font-medium hover:bg-green-200 transition-colors whitespace-nowrap"
        >
        ...
        <button
          ...
          className="px-2.5 py-1.5 bg-blue-600 text-white rounded-lg flex items-center gap-1 text-xs font-medium hover:bg-blue-700 transition-colors whitespace-nowrap"
        >
      ...
      <button
        ...
        className="px-2.5 py-1.5 bg-amber-50 text-amber-600 border border-amber-200 rounded-lg flex items-center gap-1 text-xs font-medium hover:bg-amber-100 transition-colors whitespace-nowrap"
      >
        <Eye size={13} /> View
      </button>

      <button
       onClick={() => setDeleteTarget({ student, year: currentYear })}
        className="px-2.5 py-1.5 bg-red-50 text-red-500 border border-red-200 rounded-lg flex items-center gap-1 text-xs font-medium hover:bg-red-100 transition-colors whitespace-nowrap"
      >
...
{deleteTarget.student && (
  <DeleteConfirmPopup
    ...
    onCancel={() => setDeleteTarget({ student: null, year: currentYear })}
  />
)}
